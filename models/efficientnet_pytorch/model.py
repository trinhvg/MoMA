...
from models.efficientnet_pytorch.utils import (
    round_filters,
    round_repeats,
    drop_connect,
...
class EfficientNet(nn.Module):
    ...
    def extract_features(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if drop_connect_rate:
                drop_connect_rate *= float(idx) / len(self._blocks) # scale drop connect_rate
            x = block(x, drop_connect_rate=drop_connect_rate)
            if idx in range(len(out_idx)):
                # out_dict[idx] = x
                out_dict[idx] = x
                # if idx != 2:
                #     out_dict[idx] = get_same_padding_maxPool2d()(kernel_size =2, stride=2)(x)
                # else:
                #     out_dict[idx] = x
        # Head
        x = self._swish(self._bn1(self._conv_head(x)))
    ...
    def get_feat_modules(self):
        ...
        feat_m.append(self._conv_stem)
        feat_m.append(self._bn0)
        feat_m.append(self._blocks)
        feat_m.append(self._conv_head)
        feat_m.append(self._bn1)
        feat_m.append(self.classifier_)
        return feat_m

    def forward(self, inputs, is_feat=False):
        ...
        """
        # Convolution layers
        # x, feature_maps = self.extract_features(inputs)
        out = self.extract_endpoints(inputs)


        # Pooling and final linear layer
        # x = self._avg_pooling(out['reduction_5'])
        # out['reduction_6'] = x

        x = self._avg_pooling(out[-1])
        out.append(x)
...
def efficientnet(task_mode='class', model_name='efficientnet-b0', pretrained=True, num_classes=4, **kwargs):
    """
    ...
    model = func(task_mode=task_mode, model_name=model_name , num_classes=num_classes)
    return model


# def _test():
#     net = efficientnet(pretrained=True).cuda()
