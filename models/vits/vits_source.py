...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale #torch.Size([4, 12, 197, 197])
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C) #torch.Size([4, 12, 197, 32]) => torch.Size([4, 197, 12, 32]) => torch.Size([4, 197, 384])
...
class Mlp(nn.Module):
    ...
    def forward(self, x): #torch.Size([4, 197, 384])
        x = self.fc1(x) # torch.Size([4, 197, 1536])
        x = self.act(x) #Applies the Gaussian Error Linear Units function
        x = self.drop1(x)
        x = self.fc2(x) # torch.Size([4, 197, 384])
        x = self.drop2(x)
        return x #torch.Size([4, 197, 384])
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        x = self.patch_embed(x) #torch.Size([4, 196, 384])
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks torch.Size([1, 1, 384]) = torch.Size([4, 1, 384])
        if self.dist_token is None:
            x = torch.cat((cls_token, x), dim=1) #torch.Size([4, 197, 384])
        else:
        ...
        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, 0]) # # we index only the cls token for classification. nlp tricks :P
        else:
            return x[:, 0], x[:, 1]

    def forward(self, x):
        x = self.forward_features(x) #torch.Size([4, 384])
...
def _create_vision_transformer(variant, num_classes=3, pretrained=False, default_cfg=None, **kwargs):
...
def deit_base_patch16_384(num_classes=3, pretrained=False, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('deit_base_patch16_384', num_classes=num_classes, pretrained=pretrained, **model_kwargs)
    return model
...
#
#     net = deit_base_patch16_384(num_classes=1000, pretrained=True)
#     net.head = nn.Linear(in_features=768, out_features=3, bias=True)
#     nn.init.zeros_(net.head.weight)
#     nn.init.constant_(net.head.bias, 0.)
#     net = net.cuda()
#     y_class = net(torch.randn(4, 3, 384, 384).cuda())
#     print(y_class.size())
